 ...
 * 
 * These actions affect the output components (LEDs, buzzers, relays, etc.)
 * The actions to be performed are found in a definition table.
 * 
 * @author jflamy
 *
 */
	...
	private void doPin(OutputPinDefinition d, Board board) {
		...
		try {
			...
			case "FLASH" -> 
				board.doFlash(pin, d.parameters);
			case "TONE" -> 
				board.doTones(pin, d.parameters);
			}
		} catch (Exception e) {
